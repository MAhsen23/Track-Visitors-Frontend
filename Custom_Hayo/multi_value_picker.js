...
const CustomDropdown = ({ placeholder, width, height, options, selectedValues, onValuesSelect, labelKey, valueKey }) => {
      ...
      >
        ...
        >
          {selectedValues.length
            ? selectedValues.map((item) => options.find((obj) => obj[valueKey] === item)[labelKey]).join(', ')
            : placeholder}
        </Text>
        <Image
          ...
          <View style={{
            ...
            >
              ...
              <TouchableOpacity
                ...
                onPress={() =>
                  onValuesSelect(areAllOptionsSelected ? [] : options.map((option) => option[valueKey]))
                }
              >
                <Text style={{ fontSize: 15, fontFamily: FontFamily.poppinsRegular }}>{areAllOptionsSelected ? 'Unselect All' : 'Select All'}</Text>
              </TouchableOpacity>
                    ...
                    >
                      <View
                        style={{
                          ...
                          height: 20,
                          borderWidth: 1,
                          borderColor: selectedValues.includes(option[valueKey]) ? '#3A9EBB' : 'gray',
                          marginRight: 10,
                          alignItems: 'center',
                          justifyContent: 'center',
                        }}
                      >
                        {selectedValues.includes(option[valueKey]) && (
                          <Text
                            style={{
                              marginBottom: 1, color: selectedValues.includes(option[valueKey]) ? '#3A9EBB' : 'gray',
                            }}
                          >
                            ✓
                          </Text>
